...
\vspace{1em}
\textbf{(4)} 
In the position representation, $\ket{\psi_{x_0,\epsilon}}=\int_{min}^{max} $ $$\braket{\delta | \delta}=\int_{-\infty}^{+\infty}(x\psi_{x_0,\epsilon}(x)-x_0 \psi_{x_0,\epsilon}(x))^{*}(x\psi_{x_0,\epsilon}(x)-x_0 \psi_{x_0,\epsilon}(x))dx$$
...
and 
\[
\int_{-\infty}^{+\infty}(x-x_0)^2\frac{1}{\sqrt{2\pi\sigma}}e^{-\frac{(x-x_0)^2}{2\sigma}}dx\]
\end{document}
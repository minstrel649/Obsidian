...
\textbf{(2)} The expectation value of the spin operator $S_y$ is given by:
\begin{align*}
    \braket{S_y} &= \braket{\psi(t) | S_y | \psi(t)} \\
    &= \cos^2(\theta) \braket{\uparrow | S_y | \uparrow} + \sin^2(\theta) \braket{\downarrow | S_y | \downarrow} + 2\cos(\theta)\sin(\theta) \braket{\uparrow | S_y | \downarrow}
\end{align*}
\end{document}